...
export default function RunMonitor() {
  ...
  if (loading || !run) return (
    <div className="flex items-center gap-4 mt-12 animate-in">
      ...
      <span style={{ color: 'var(--on-surface-variant)', fontSize: '14px' }}>Connecting...</span>
    </div>
  )
      ...
      <div className="page-header flex justify-between items-end">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span style={{ fontSize: '12px', color: 'var(--on-surface-variant-muted)' }}>
              {run.id.split('-')[0]}
            </span>
          </div>
        </div>
        ...
        {run.status === 'running' && (
          <button className="btn btn-secondary" style={{ color: 'var(--error)', borderColor: 'var(--error)' }} onClick={handleStop}>
            <Square size={14} /> Stop
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px', marginBottom: '24px' }}>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            padding: '4px 10px', background: 'var(--surface-container)', 
            border: '1px solid var(--outline-variant)', borderRadius: 'var(--radius-xs)',
        ...
        <div className="flex flex-col gap-4">
          <div className="card" style={{ flex: 1, maxHeight: '300px', overflowY: 'auto' }}>
            <div className="panel-header">Run Details</div>
            
            <div className="flex flex-col gap-6">
              <div>
                <div className="text-sm text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>Mode</div>
                <div style={{ color: 'var(--on-surface)', fontSize: '15px', fontWeight: 500 }}>{run.mode}</div>
              </div>

              <div>
                <div className="text-sm text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>Started</div>
              ...
              <div>
                <div className="text-sm text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>Configuration</div>
              ...
              <div style={{ borderTop: '1px solid var(--outline-variant)', paddingTop: 16 }}>
                <div className="text-sm text-muted" style={{ fontSize: '11px', textTransform: 'uppercase', marginBottom: '12px' }}>Progress</div>
          ...
          <div className="card" style={{ flex: 1, maxHeight: '334px', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div className="panel-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Live Feed</span>
              {feedEvents.length > 0 && <span style={{ color: 'var(--success)', fontSize: '11px' }}>Active</span>}
            </div>
            
            <div className="flex flex-col gap-3" style={{ flex: 1 }}>
              {feedEvents.length === 0 ? (
                <div style={{ opacity: 0.5, fontSize: '12px', color: 'var(--on-surface-variant-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                  Waiting for data...
                </div>
              ) : (
                [...feedEvents].reverse().map((evt) => (
                  <div key={evt.id || evt.timestamp || `${evt.type}-${evt.round}`} className={`feed-item ${evt.type === 'tool_result' ? 'feed-item-accent' : 'feed-item-neon'}`}>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: evt.type === 'tool_result' ? 'var(--primary)' : 'var(--tertiary)', marginBottom: '4px', display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        {evt.type === 'tool_result' ? <Wrench size={12} /> : <Rss size={12} />}
                        {evt.type === 'tool_result' ? evt.toolId : evt.sourceType}
                      </span>
                      <span style={{ color: 'var(--on-surface-variant-muted)' }}>R{evt.round}</span>
                    </div>
                    {evt.type === 'tool_result' ? (
                      <div>
                        <div style={{ fontSize: '11px', color: 'var(--on-surface)', marginBottom: '4px', fontStyle: 'italic' }}>"{evt.query}"</div>
                        <div style={{ fontSize: '10px', color: 'var(--on-surface-variant-muted)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{evt.resultPreview}</div>
                      </div>
                    ) : (
                      <div>
          ...
          {run.error && (
            <div className="card" style={{ borderColor: 'var(--error)', background: 'var(--error-container)' }}>
              <div style={{ fontSize: '12px', color: 'var(--error)', fontWeight: 600, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}><AlertCircle size={14} /> Error</div>
          ...
          {run.log ? (
            run.log.split('\n').map((line, i) => (
              <div key={i} className="log-line" style={{ fontSize: '11px' }}>
                <span style={{ color: 'var(--on-surface-variant-muted)', marginRight: '12px' }}>[{i.toString().padStart(4, '0')}]</span>
                {line}
              </div>
            ))
          ) : (
            <div style={{ color: 'var(--on-surface-variant-muted)', fontSize: '12px' }}>No console output yet...</div>
          )}
        </div>
      </div>
    </div>
